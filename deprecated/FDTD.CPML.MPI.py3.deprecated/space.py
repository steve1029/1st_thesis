...
class Space(object):
	...
	def __init__(self, grid, gridgap, dtype, **kwargs):
		...
		self.TOTAL_NUM_GRID_SIZE = (self.dtype(1+1j).nbytes * self.TOTAL_NUM_GRID) / 1024 / 1024
	...
	def updateH(self,tstep) :
		...
		if self.MPIrank >= 1 and self.rank <= (self.MPIsize-1):
			...
			self.MPIcomm.send( send_Ey_first, dest=(self.MPIrank-1), tag=(tstep*100+5))
			self.MPIcomm.send( send_Ez_first, dest=(self.MPIrank-1), tag=(tstep*100+6))
		...
		if self.MPIrank >= 0 and self.rank <= (self.MPIsize-2):
			...
			recv_Ey_last = self.MPIcomm.recv( source=(self.MPIrank+1), tag=(tstep*100+5))
			recv_Ez_last = self.MPIcomm.recv( source=(self.MPIrank+1), tag=(tstep*100+6))
	...
	def updateE(self, tstep) :
		...
		if self.MPIrank > 0 and self.rank < (self.MPIsize-1): # rank 1,2,3,...,n-2
			...
			self.MPIcomm.send(send_Hx_last, dest=(self.MPIrank+1), tag=(tstep*100+1))
			self.MPIcomm.send(send_Hy_last, dest=(self.MPIrank+1), tag=(tstep*100+2))
		...
		if self.MPIrank > 1 and self.rank < self.MPIsize: # rank 2,3,...,n-1
			...
			recv_Hx_first = self.MPIcomm.recv( source=(self.MPIrank-1), tag=(tstep*100+1))
			recv_Hy_first = self.MPIcomm.recv( source=(self.MPIrank-1), tag=(tstep*100+2) )
