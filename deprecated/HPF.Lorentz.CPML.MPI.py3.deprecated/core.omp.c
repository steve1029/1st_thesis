	...
	for(i=0; i < myNx; i++){
		for(j=0; j < Ny; j++){
			for(k=0; k < Nz; k++){
				...
				CEx2 =	(2.*dt) / (2.*eps_EHH[myidx]);
				CEy2 =	(2.*dt) / (2.*eps_HEE[myidx]);
				CEz2 =	(2.*dt) / (2.*eps_HEE[myidx]);

				// Update Ex.
				Ex_re[myidx] = CEx1 * Ex_re[myidx] + CEx2 * (diffyHz_re[myidx] - diffzHy_re[myidx]);
				Ex_im[myidx] = CEx1 * Ex_im[myidx] + CEx2 * (diffyHz_im[myidx] - diffzHy_re[myidx]);
	...
	for(i=0; i < myNx; i++){
		for(j=0; j < Ny; j++){
			for(k=0; k < Nz; k++){
				...
				CHx2 =	(-2*dt) / (2.*mu_HEE[myidx]);
				CHy2 =	(-2*dt) / (2.*mu_EHH[myidx]);
				CHz2 =	(-2*dt) / (2.*mu_EHH[myidx]);
