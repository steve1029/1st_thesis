...
void update_inner_E(double *Ex_re , double *Ex_im , double *Ey_re , double *Ey_im, double *Ez_re, double *Ez_im, \
	...
	#pragma omp parallel for \
		...
		private(i,j,k, myidx, myidx_j, myidx_k, diffyHz_re, diffyHz_im, diffzHy_re, diffzHy_im)
	for(i=0; i < myNx; i++){
		for(j=1; j < Ny; j++){
			for(k=1; k < Nz; k++){
	...
	#pragma omp parallel for \
		...
		private(i,j,k, myidx, myidx_i, myidx_k, diffxHz_re, diffxHz_im, diffzHx_re, diffzHx_im)
	for(i=1; i < myNx; i++){
		for(j=0; j < Ny; j++){
			for(k=1; k < Nz; k++){
	...
	#pragma omp parallel for \
		...
		private(i,j,k, myidx, myidx_i, myidx_j, diffxHy_re, diffxHy_im, diffyHx_re, diffyHx_im)
	for(i=1; i < myNx; i++){
		for(j=1; j < Ny; j++){
			for(k=0; k < Nz; k++){
...
void update_inner_H(double *Hx_re, double *Hx_im, double *Hy_re, double *Hy_im, double *Hz_re, double *Hz_im, \
	...
	#pragma omp parallel for \
		...
		private(i,j,k, myidx, j_myidx, k_myidx, diffyEz_re, diffyEz_im, diffzEy_re, diffzEy_im)
	for(i=0; i < myNx; i++){
		for(j=0; j < (Ny-1); j++){
			for(k=0; k < (Nz-1); k++){
	...
	#pragma omp parallel for \
		...
		private(i,j,k, myidx, i_myidx, k_myidx, diffxEz_re, diffxEz_im, diffzEx_re, diffzEx_im)
	for(i=0; i < (myNx-1); i++){
		for(j=0; j < Ny; j++){
			for(k=0; k < (Nz-1); k++){
	...
	// Update Hz
	#pragma omp parallel for \
		shared(myNx, Ny, Nz, dt, dx, dz, Ex_re, Ex_im, Ey_re, Ey_im, Hz_re, Hz_im, mu_Hz)	\
		private(i,j,k, myidx, i_myidx, j_myidx, diffxEy_re, diffxEy_im, diffyEx_re, diffyEx_im)
	for(i=0; i < (myNx-1); i++){
		for(j=0; j < (Ny-1); j++){
			for(k=0; k < Nz; k++){
...
void update_first_EyEz(	double *Ey_re, double *Ey_im, double *Ez_re, double *Ez_im, \
	...
	// Update Ey at x=0
	#pragma omp parallel for \
		shared(Ny, Nz, dt, dx, dz, Hx_re, Hx_im, Hz_re, Hz_im, recvHzfirst_re, recvHzfirst_im, Ey_re, Ey_im, eps_Ey)	\
		private(j,k, myidx, myidx_k, diffxHz_re, diffxHz_im, diffzHx_re, diffzHx_im)
	for(j=0; j < Ny; j++){
		for(k=1; k < Nz; k++){
	...
	// Update Ez at x=0
	#pragma omp parallel for \
		shared(Ny, Nz, dt, dx, dy, Hx_re, Hx_im, Hy_re, Hy_im, recvHyfirst_re, recvHyfirst_im, Ez_re, Ez_im, eps_Ez)	\
		private(j,k, myidx, myidx_j, diffxHy_re, diffxHy_im, diffyHx_re, diffyHx_im)
	for(j=1; j < Ny; j++){
		for(k=0; k < Nz; k++){
...
void update_last_HyHz(	double *Hy_re, double *Hy_im, double *Hz_re, double *Hz_im, \
	...
	#pragma omp parallel for \
		shared(Ny, Nz, dt, dx, dz, Ex_re, Ex_im, Ez_re, Ez_im, recvEzlast_re, recvEzlast_im, Hy_re, Hy_im, mu_Hy)	\
		private(j,k, myidx, k_myidx, yzidx, diffxEz_re, diffxEz_im, diffzEx_re, diffzEx_im)
	for(j=0; j < Ny; j++){
		for(k=0; k < (Nz-1); k++){
	...
	#pragma omp parallel for \
		shared(Ny, Nz, dt, dx, dy, Ex_re, Ex_im, Ey_re, Ey_im, recvEylast_re, recvEylast_im, Hz_re, Hz_im, mu_Hz)	\
		private(j,k, myidx, j_myidx, yzidx, diffxEy_re, diffxEy_im, diffyEx_re, diffyEx_im)
	for(j=0; j < (Ny-1); j++){
		for(k=0; k < Nz; k++){
